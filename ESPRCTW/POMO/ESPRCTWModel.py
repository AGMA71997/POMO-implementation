...
class ESPRCTW_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, load, current_time, current_prices, ninf_mask):
        ...
        #######################################################
        input_cat = torch.cat((encoded_last_node, load[:, :, None], current_time[:, :, None], current_prices[:, :, None]), dim=2)
